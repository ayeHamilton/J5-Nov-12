...
public class CustomerDBProcess
{
    ...
    public void updateCustomer(CustomerDBObject cust)throws SQLException{
	int cust_id = cust.getCustomerId();
	String f_name = cust.getFirstName();
	String l_name = cust.getLastName();
	int age = cust.getAge();
	...
	statement.executeUpdate("UPDATE customer SET first_name='"+f_name+"', last_name='"+l_name+"', age="+age+", order_value="+order_val+" WHERE customer_id="+cust_id+"");
	
    }

    public CustomerDBObject getCustomer(String custid)throws SQLException{
	CustomerDBObject cust = new CustomerDBObject();

	rs = statement.executeQuery("SELECT * FROM CUSTOMER WHERE customer_id="+custid+"");
